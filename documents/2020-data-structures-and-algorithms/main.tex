...
\section{Algorithms and Techniques}
    \subsection{Sequence Search and Sorting}
...
\end{lstlisting}
    ...
    A possible variation is to use two sliding windows. This may be necessary when a question asks to find the number of valid sub-sequence of \textit{exactly} $K$ elements. Then we want to find $exactly(K) = atMost(K) - atMost(K-1)$ and may define two sliding windows, one which counts subsequences with length $<= k$ and another with length $<= k-1$.
