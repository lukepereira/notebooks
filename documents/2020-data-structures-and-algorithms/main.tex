...
\end{lstlisting}
    ...
    Breadth-first search will find you the path with the fewest segments. But in Dijkstra’s algorithm, you assign a number or weight to each segment. Then Dijkstra’s algorithm finds the path with the smallest total weight.
...
\end{lstlisting}
    ...
    \begin{itemize}
        ...
        \item \textbf{Set bit}  -- $A \vert= 1 <<  \text{bit}$
    ...
    \begin{enumerate}
        ...
        $x \And (x-1)$ will have all the bits equal to the $x$ except for the rightmost $1$ in $x$.
...
\end{lstlisting}
    ...
    A recursively defined sequence with a strong relation to the golden ratio.
...
\end{lstlisting}
    ...
    The time complexity of a brute-force computation of a Catalan number is $\bigO(3^n)$ which can be reduced to $\bigO(2^n)$ if recursive calls are memoized.
...
\begin{itemize}
    \item  Best Conceivable Runtime (BCR). Try to derive an approach from an ideal upper bound on the solution
    \item Consider making a space-time tradeoff, usually in the form of a hashtable or cached results
    \item Data Structure Brainstorm. Linked List, Array, Stack, Queue, Heap, Binary tree, Hashtables, etc.
    \item Simplify and Generalize. Simplify problem statement then attempt to generalize solution
    \item Look for BUD (bottlenecks, unnecessary work, duplicated work)
    \item DIY (Do It Yourself). Design an algorithm around how you would solve an analgous real-word scenario without programming
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Can you pre-process the input or re-interpret the desired output solution to simplify or reduce the problem?
        \item What are problematic or challenging areas that might arise from certain inputs?
        What are your intuitions about possible solutions? You can revisit these ideas later.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Repeat or expand, always spend extra time considering alternative approaches before settling on solution
        \item When out of ideas or if able to meet best conceivable runtime, prompt interviewer for approval: ``If you're happy with this approach, I can go into the finer details and begin implemention''
    \end{itemize}
    
     \item Consider more granular implementation details

    \begin{itemize}
        \item Describe edge cases: (ex: empty input, invalid inputs, large inputs)
        \item Consider boundary conditions if dealing with iterations, indexed arrays, ranges
        \item Describe sufficiently complex test case if none are given, consider problem areas
        \item Reconsider minor optimizations to general approach (short-circuiting, data structures, caching)
        \item If using recursion, note limitations of relying on call stack
    \end{itemize}
    
    \item Implement
    \begin{itemize}
        \item Speak aloud your thinking process, even when stuck
        \item Add base case, i.e. empty inputs, invalid input error checks
