...
\end{lstlisting}
    ...
    Time Complexity: $\bigO(Q + n)$, Space Complexity: $\bigO(1)$
    
    Given an array $A$ of 0's of size $n$, perform $Q$ operations or queries by incrementing values in the subarray $A[L: R]$ by 1. A naive brute force solution will result in time complexity of $\bigO(Q\cdot n)$. 
    
    Using a numerical method, we are able to reduce the time complexity to $\bigO(Q + n)$. The technique involves creating a secondary array $B$ and only incrementing the value at the left endpoint, $L$ by 1 and decrementing the value at index $R + 1$ by 1. After repeating this process for all queries, to find the true desired value of $A[i]$ we can find the prefix sum of $B$ from $B[0:i]$.
    
    We can find a maximum in the array in $\bigO(n)$ by storing the difference between the current element and the previous element. The means that we add the sum to $A[i]$ showing that $A[i]$ is greater than its previous element by the sum. We subtract sum from $A[j+1]$ to show that $a[j+1]$ is less than $a[j]$ by sum (since $a[j]$ was the last element that was added to sum). By the end of all this, we have an array that shows the difference between every successive element. By adding all the positive differences, we get the value of the maximum element
